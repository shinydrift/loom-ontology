...
- [ ] **`or` / `in` / `not` in a caller's filter** — M7 built `AND` because `Search.filters` is
      already a conjunction; these three are not, and each needs an IR shape (a tree rather than a
      tuple), an engine lowering and a `pushdown_hints` answer — a hint derived from one arm of an
      `or` is wrong unless the `WHERE` still re-applies the whole thing. `in` is the one an agent
      misses first: without it, N values are N calls. Kleene propagation is settled already, so the
      null question does not reopen — but `not` does reopen the one thing M7 leaned on, that this
      grammar has no negation, and would have to answer it the way `predicate.py` does.
