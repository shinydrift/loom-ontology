...
class Resolver:
    ...
    def _filters(self, obj: ObjectType, filters: Mapping[str, Any]) -> tuple[Comparison, ...]:
        ...
        **`searchable` is checked here and not only at the surface.** It used to be enforced by
        `_search_tool` alone — which builds the `filter` schema from the list — so a caller who
        ignored the advertised schema could filter on any declared property, and `filter`'s
        `additionalProperties: false` was an announcement nothing made true. §2 rule 6 is the
