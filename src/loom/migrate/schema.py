...
from ..model import LinkType, ObjectType, Ontology, physical_type
...
def _add_object(
    ...
    for prop in obj.properties.values():
        ...
        _merge(
            ...
            obj.backing_catalog,
            obj.backing_table,
            DesiredColumn(prop.column, stored, required=not prop.nullable, source=f"{obj.api_name}.{prop.name}"),
            owner=obj.api_name,
            diag=diag,
        )
...
def _add_through(
    ...
    if link.through is None:
        return
    for column, end in ((link.through.from_column, link.frm), (link.through.to_column, link.to)):
        ...
        _merge(
            ...
            link.through.table,
            DesiredColumn(column, stored, required=True, source=f"{link.api_name}.{end.object_type}"),
            owner=link.api_name,
            diag=diag,
        )
...
def _reconcile(prior: DesiredColumn, new: DesiredColumn, table: str, diag: Diagnostics) -> DesiredColumn:
    ...
    if prior.iceberg_type != new.iceberg_type:
        ...
        return prior
    return replace(prior, required=prior.required and new.required)
